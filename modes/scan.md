...
7.5. **Verify liveness of WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   **Do not stop the whole scan if one URL fails.** If `browser_navigate` errors (timeout, 403, etc.), mark as `skipped_expired` and continue.
...
Each company in `tracked_companies` must have `careers_url` — the direct URL to their jobs page. This avoids looking it up every time.
...
1. Try the pattern from its known platform
